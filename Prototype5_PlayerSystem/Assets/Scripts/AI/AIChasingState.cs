...
public class AIChasingState : AIState
{
    ...
    public override AIState Tick(EnemyManager em, EnemyStats es, EnemyAnimationManager eam)
    {
        ...
        Vector3 targetDirection = em.character.transform.position - em.transform.position;
        em.distanceFromTarget  = Vector3.Distance(em.character.transform.position, em.transform.position);
        float viewAbleAngle = Vector3.Angle(targetDirection, em.transform.forward);
    ...
    private void HandleRotateTowardsTarget(EnemyManager em)
    {
        if (em.isPerformingAction)
        {
            Vector3 direction = em.character.transform.position - em.transform.position;
            direction.y = 0;
            direction.Normalize();
            ...
            em.transform.rotation = Quaternion.Slerp(em.transform.rotation, targetRotation, em.rotationSpeed / Time.deltaTime);
        }
        else
        {
            ...
            em.transform.rotation = Quaternion.Slerp(em.transform.rotation, em.navmeshAgent.transform.rotation, em.rotationSpeed / Time.deltaTime);
        }
